...
bond_hashstop(){
	$ssdk_sh misc cpuVid set disable >/dev/null
	$ssdk_sh trunk group set 0 disable 0x41 >/dev/null
	$ssdk_sh trunk hashmode set 0x0 >/dev/null
}
...
sw_printconf_add_port() # $1: device, $2: port, $3: pvid
{
	cat <<EOF
config switch_port
	option device '$1'
...
sw_config_ct_vlan()
{
	...
	ct_vlan_num=$($CONFIG get ct_vlan_num)
	[ $ct_vlan_num -lt 1 ] && return

	index=0
	while [ $index -lt $ct_vlan_num ]; do
		vid=$($CONFIG get ct_vlan_vid_x${index})
		phyports=$($CONFIG get ct_vlan_phyports_x${index})
		sw_printconf_add_vlan "switch0" "$((index + 1))" "$vid" "$phyports" >> $swconf
		index=$((index + 1))
	done
}
...
sw_config_ct_port()
{
	...
	while [ $index -lt 16 ]; do
		pvid=$($CONFIG get ct_port_pvid_x$index)
		[ -z $pvid ] && break
		sw_printconf_add_port "switch0" "$index" "$pvid" >> $swconf
		index=$((index + 1))
	done
}

sw_config_ct_port_fix()
{
	index=1
	while [ $index -lt 16 ]; do
		pvid=$($CONFIG get ct_port_pvid_x$index)
		[ -z $pvid ] && break
		$ssdk_sh portVlan defaultCvid set $index $pvid >/dev/null 2>&1
		index=$((index + 1))
	done
}
...
sw_configvlan_normal()
{
	...
	ct_vlan_en=$($CONFIG get ct_vlan_en)
...
sw_flowctrl()
{
	...
	$ssdk_sh port flowctrlforcemode set 5 enable >/dev/null
	$ssdk_sh port flowctrl set 5 enable >/dev/null
	$ssdk_sh debug reg set 0x7c 0x17e 4 >/dev/null
	$ssdk_sh debug reg set 0x94 0x17e 4 >/dev/null
	$ssdk_sh debug reg set 0x808 0x7f004e 4 >/dev/null
}
