...
normal_multi_lan_vifs()
{
	...
	while [ $index -lt $lan_num ]; do
		if [ $index -eq 0 ]; then
			vid=$($CONFIG get lan_vid)
			mac=$($CONFIG get lan_factory_mac)
		else
...
normal_multi_wan_vifs()
{
	...
	while [ $index -lt $wan_num ]; do
		if [ $index -eq 0 ]; then
			vid=$($CONFIG get wan_vid)
			mac=$($CONFIG get wan_factory_mac)
			brx=brwan
		else
			vid=$($CONFIG get ct_wan_vid_x$index)
			mac=$($CONFIG get ct_wan_macaddr_x$index)
			brx=brwan$index
			br_create $brx
		fi
		vconfig add ethwan $vid
		brctl addif $brx ethwan.$vid
		ifconfig $brx hw ether $mac
		index=$((index + 1))
	done
}
